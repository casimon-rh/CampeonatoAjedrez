...
{
    public class RutinasHoteles : IRutinasCatalogos<Hotel>, INotifyPropertyChanged
    {
        ...
        Window.Catalogos.Hoteles window;
        ...
        public void MetroWindow_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                window = sender as Window.Catalogos.Hoteles;
                this.daoCatalogo  = Service.getdaoDiccionario().daos["CatalogoHotel"] as ICatalogos<Hotel>;
                Current = new Hotel();
                Catalogos = daoCatalogo.getAll();


                (sender as Window.Catalogos.Hoteles).Flip.HideControlButtons();
            }
            catch (Exception ex)
            {
